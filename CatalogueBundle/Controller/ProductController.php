...
use CatalogueBundle\Form\ProductRelatedType;
...
use CatalogueBundle\Form\ProductType;
use CatalogueBundle\Form\ProductAttributesType;
use CatalogueBundle\Form\ProductFeaturesType;
...
class ProductController extends Controller
{
     ...
     * @Route("/")
     * @Method("GET")
     * @Template("CatalogueBundle:Product:index.html.twig")
     */
    ...
    public function listJsonAction()
    {
        ...
        $jsonList->setRepository($em->getRepository('CatalogueBundle:Product'));
    ...
    public function statsAction($id, $from, $to)
    {
        ...
        /** @var Actor $entity */
        $entity = $em->getRepository('CatalogueBundle:Product')->find($id);
    ...
    public function newAction(Request $request)
    {
        ...
        $entity = new Product();
        $form = $this->createForm('CatalogueBundle\Form\ProductType', $entity, $formConfig);
        ...
        if ($form->isSubmitted() && $form->isValid()) {
            ...
            return $this->redirectToRoute('catalogue_product_show', array('id' => $entity->getId()));
        }
    ...
    public function showAction(Product $product)
    {
        ...
        //disable notification
        $em = $this->getDoctrine()->getManager();
        $admin = $em->getRepository('CoreBundle:BaseActor')->find(1);//admin
        $notificationManager = $this->container->get('notification_manager');
        $notificationManager->disableNotificationByDetail(
                $admin, //current user
                $admin,//admin
    ...
    public function editAction(Request $request, Product $product)
    {
        ...
        if ($user->isGranted('ROLE_USER') && $product->getActor()->getId() != $user->getId()) {
            return $this->redirect($this->generateUrl('catalogue_product_index'));
        }
        ...
        $editForm = $this->createForm('CatalogueBundle\Form\ProductType', $product, $formConfig);
        $attributesForm = $this->createForm('CatalogueBundle\Form\ProductAttributesType', $product, array('category' => $product->getCategory()->getId()));
        $featuresForm = $this->createForm('CatalogueBundle\Form\ProductFeaturesType', $product, array('category' => $product->getCategory()->getId()));
        $relatedProductsForm = $this->createForm('CatalogueBundle\Form\ProductRelatedType', $product);
        ...
        if($request->getMethod('POST')){
            ...
            if ($editForm->isSubmitted() && $editForm->isValid()) {
                ...
                return $this->redirectToRoute('catalogue_product_show', $redirectParams);
            }
        }
    ...
    {
        $em = $this->getDoctrine()->getManager();
        $totalVisits = $em->getRepository('CatalogueBundle:Product')->countTotalStatsVisits();
        $totalActorVisited = $em->getRepository('CatalogueBundle:Product')->countProductVisited();
        $totalActors = $em->getRepository('CatalogueBundle:Product')->countTotal();
        $bounceRate = $em->getRepository('CatalogueBundle:Product')->bounceRate();
        $stats = $em->getRepository('CatalogueBundle:Product')->countProductStatsLine();
        $report = $em->getRepository('CatalogueBundle:Product')->countVisitsByProduct();
        $cuponCount = $em->getRepository('CatalogueBundle:Transaction')->getCountCupon();
        $bestSeller = $em->getRepository('CatalogueBundle:Transaction')->getBestSellerProducts(1);
        $totalAmount = $em->getRepository('CatalogueBundle:Transaction')->totalAmountCharged();
        $totalTransactions = $em->getRepository('CatalogueBundle:Transaction')->countTotal();
   ...
   /**
    ...
    private function createDeleteForm(Product $product)
    {
        return $this->createFormBuilder()
            ->setAction($this->generateUrl('catalogue_product_delete', array('id' => $product->getId())))
            ->setMethod('DELETE')
            ->getForm()
        ;
    }
